...
#pragma omp parallel
  {
    ...
    // Chaque thread annonce qu'il s'apprète à écrire dans son fichier
    // avant de le faire (évite une race condition sur la sortie standard).
#pragma omp critical
    {
      printf("thread %d: going to print ...\n", tid);
